...
class PassengerManager {
    ...
    async checkEmail(email) {
        let checkquery = `SELECT * FROM passengers WHERE email = '${email}'`;
        let result = await connection.query(checkquery);
        return result[0][0];
    }
    ...
    async update(firstname, lastname, email, phone_no, age, sex) {
        let updateQuery = `UPDATE passengers SET firstname = '${firstname}', lastname ='${lastname}',email = '${email}', phone_no = '${phone_no}',age=${age},sex='${sex}' WHERE email = '${email}'`;
